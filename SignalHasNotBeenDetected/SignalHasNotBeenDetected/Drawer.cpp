...
void Drawer::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	float kof_y_transform = (lpDrawItemStruct->rcItem.bottom - lpDrawItemStruct->rcItem.top + 1.0f - free_place_kof) / (2.0f * MAX_A);
	Matrix M;
	...
	float delta_y_greed = 2.0f * MAX_A / (float)greed_size;
	...
	for (int i = 0; i <= greed_size; i++)
	{
		...
		float y = i * delta_y_greed - MAX_A;
		Gr.DrawLine(&YunYun_Pen, x, -MAX_A, x, MAX_A);
		...
		PointF x_cor(x, -MAX_A), y_cor(0, y);
	...
	PointF* Plot1 = new PointF[n];
	PointF* Plot2 = new PointF[n];
	...
	Gr.DrawCurve(&Megumin_Pen, Plot1, n);
	Gr.DrawCurve(&Aqua_Pen, Plot2, n);
